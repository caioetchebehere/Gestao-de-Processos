(async function () {
  ...
  const CHAVE_CLIENT_ID = 'gp-client-id';
  ...
  function obterClientId() {
    let id = localStorage.getItem(CHAVE_CLIENT_ID);
    const valido = id && /^[a-f0-9]{32}$/.test(id);
    if (!valido) {
      id = (typeof crypto !== 'undefined' && crypto.randomUUID)
        ? crypto.randomUUID().replace(/-/g, '')
        : Array.from({ length: 32 }, () => Math.floor(Math.random() * 16).toString(16)).join('');
      localStorage.setItem(CHAVE_CLIENT_ID, id);
    }
    return id;
  }

  function mostrarCarregando(visivel) {
    const el = document.getElementById('app-carregando');
  ...
  async function salvar() {
    ...
    try {
      const res = await fetch('/api/data', {
        ...
        body: JSON.stringify({ clientId: obterClientId(), state: payload })
      });
      if (!res.ok) {
      ...
      mostrarToast('Erro ao salvar no servidor: ' + e.message, 'erro');
    }
  }
  ...
  async function carregar() {
    try {
      ...
      const res = await fetch('/api/data?clientId=' + encodeURIComponent(clientId));
      if (!res.ok) throw new Error('HTTP ' + res.status);
      const json = await res.json();
      const data = json.data;

      if (!data || typeof data !== 'object') {
        // Primeira visita — gravar dados de exemplo no servidor
      ...
      // Sem conexão — usar dados de exemplo em memória (não salvar)
      ...
      mostrarToast('Não foi possível conectar ao servidor. Os dados não serão salvos.', 'erro');
    }
  }
  ...
  // --- DOM e referências ---
  const ref = {
    qtdProcessos: document.getElementById('qtd-processos'),
    listaStatusPainel: document.getElementById('lista-status-painel'),
    ...
    // Modais
    modalNovoProcesso: document.getElementById('modal-novo-processo'),
    modalNomeProcessoInput: document.getElementById('modal-nome-processo'),
    modalErroNome: document.getElementById('modal-erro-nome'),
    modalBtnConfirmarNovo: document.getElementById('modal-btn-confirmar-novo'),
    modalBtnCancelarNovo: document.getElementById('modal-btn-cancelar-novo'),
    modalConfirmar: document.getElementById('modal-confirmar'),
    modalConfirmarMsg: document.getElementById('modal-confirmar-msg'),
  ...
  // --- Modais ---
  let _modalNovoCallback = null;
  let _modalConfirmarCallback = null;
  let _modalConfirmarOrigem = null;

  function abrirModalNovoProcesso() {
    ref.modalNomeProcessoInput.value = '';
    ref.modalErroNome.textContent = '';
    ref.modalNovoProcesso.hidden = false;
    ref.modalNomeProcessoInput.focus();
  }

  function fecharModalNovoProcesso() {
    ref.modalNovoProcesso.hidden = true;
    ref.btnNovoProcesso.focus();
    _modalNovoCallback = null;
  }

  function abrirModalConfirmar(mensagem, callback, origemEl) {
    ref.modalConfirmarMsg.textContent = mensagem;
    ref.modalConfirmar.hidden = false;
    _modalConfirmarCallback = callback;
    _modalConfirmarOrigem = origemEl || null;
    ref.modalBtnOk.focus();
  }

  function fecharModalConfirmar() {
    ref.modalConfirmar.hidden = true;
    if (_modalConfirmarOrigem) _modalConfirmarOrigem.focus();
    _modalConfirmarCallback = null;
    _modalConfirmarOrigem = null;
  }

  ref.modalBtnConfirmarNovo.addEventListener('click', () => {
    const nome = (ref.modalNomeProcessoInput.value || '').trim();
    if (!nome) {
      ref.modalErroNome.textContent = 'Informe um nome para o processo.';
      ref.modalNomeProcessoInput.focus();
      return;
    }
    fecharModalNovoProcesso();
    const novo = criarProcesso({ nome, dataGoLive: '', etapas: [] });
  ...
  ref.modalNomeProcessoInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') ref.modalBtnConfirmarNovo.click();
    if (e.key === 'Escape') fecharModalNovoProcesso();
  });

  ref.modalBtnCancelarNovo.addEventListener('click', fecharModalNovoProcesso);

  ref.modalNovoProcesso.addEventListener('click', (e) => {
    if (e.target === ref.modalNovoProcesso) fecharModalNovoProcesso();
  });

  ref.modalBtnOk.addEventListener('click', () => {
    const cb = _modalConfirmarCallback;
    fecharModalConfirmar();
    if (cb) cb();
  });

  ref.modalBtnCancelar.addEventListener('click', fecharModalConfirmar);

  ref.modalConfirmar.addEventListener('click', (e) => {
    if (e.target === ref.modalConfirmar) fecharModalConfirmar();
  });

  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      if (!ref.modalNovoProcesso.hidden) fecharModalNovoProcesso();
      if (!ref.modalConfirmar.hidden) fecharModalConfirmar();
    }
  });

  function atualizarPainelProcessos() {
    if (ref.qtdProcessos) {
      ref.qtdProcessos.textContent = String(estado.processos.length);
    }
    if (!ref.listaStatusPainel) return;

    const contagem = {};
    STATUS_ETAPA.forEach(status => {
      contagem[status] = 0;
    });
    estado.processos.forEach(proc => {
    ...
    ref.listaStatusPainel.innerHTML = STATUS_ETAPA.map(status =>
  ...
  function validarFormEtapa() {
    ...
    if (nome.length < 3) {
      ...
      ok = false;
    } else if (proc && Array.isArray(proc.etapas)) {
      const nomeNormalizado = nome.toLocaleLowerCase('pt-BR');
      const existeDuplicado = proc.etapas.some(etapa =>
        etapa.id !== idEtapaEditando &&
        String(etapa.nome || '').trim().toLocaleLowerCase('pt-BR') === nomeNormalizado
      );
      if (existeDuplicado) {
        erros.nomeEtapa.textContent = 'Esta etapa já foi cadastrada neste processo.';
        ref.nomeEtapa.classList.add('invalido');
        ref.nomeEtapa.parentElement.classList.add('invalido');
        ok = false;
      }
  ...
  function renderizarTabela() {
    ...
    ordenadas.forEach((etapa, idx) => {
      ...
      tr.innerHTML =
        ...
        '<td class="col-status">' + badgeStatus(etapa.status || STATUS_ETAPA_PADRAO) + '</td>' +
        '<td class="acoes">' +
  ...
  const _STATUS_CLASSE = {
    'Não iniciado': 'badge-nao-iniciado',
  ...
  function badgeStatus(status) {
    const cls = _STATUS_CLASSE[status] || 'badge-nao-iniciado';
    return '<span class="badge-status ' + cls + '">' + escapeHtml(status || STATUS_ETAPA_PADRAO) + '</span>';
  }
  ...
  ref.btnLimparTudo.addEventListener('click', () => {
    ...
    if (!proc.etapas || proc.etapas.length === 0) {
      mostrarToast('Não há etapas para remover.', 'erro');
      return;
    }
    abrirModalConfirmar(
      'Remover todas as etapas do processo "' + proc.nome + '"? Esta ação não pode ser desfeita.',
      () => {
        proc.etapas = [];
        salvar();
        renderizarTabela();
  ...
  ref.btnExcluirProcesso.addEventListener('click', () => {
    ...
    abrirModalConfirmar(
      'Excluir o processo "' + proc.nome + '" e todas as suas etapas? Esta ação não pode ser desfeita.',
      () => {

(function () {
  ...
  const DEBOUNCE_REMOTE_MS = 450;

  /** 'blob' = Vercel Blob via API; 'local' = localStorage (fallback sem token / erro de rede) */
  ...
  function salvarLocalStorage() {
    try {
      localStorage.setItem(CHAVE_STORAGE, JSON.stringify(serializarEstado()));
    } catch (e) {
      mostrarToast('Erro ao salvar no navegador.', 'erro');
    }
  }

  async function flushSalvarRemoto() {
    const r = await fetch(API_STATE, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json; charset=utf-8' },
      body: JSON.stringify(serializarEstado())
    });
    if (r.status === 503) {
      storageBackend = 'local';
      salvarLocalStorage();
      mostrarToast('Armazenamento na nuvem indisponível; dados salvos só neste navegador.', 'erro');
      return;
    }
  ...
  function salvar() {
    if (storageBackend === 'blob') {
      ...
      remoteSaveTimer = setTimeout(() => {
        ...
        flushSalvarRemoto().catch(() => {
          mostrarToast('Erro ao salvar na nuvem. Verifique a conexão e tente de novo.', 'erro');
        });
      }, DEBOUNCE_REMOTE_MS);
      return;
    }
  ...
  function flushSalvarRemotoSeNuvem() {
    if (storageBackend !== 'blob') return;
    if (remoteSaveTimer) {
      clearTimeout(remoteSaveTimer);
      remoteSaveTimer = null;
    }
    return flushSalvarRemoto().catch(() => {});
  }
  ...
  function aplicarPayloadCarregado(data) {
  ...
  async function carregarRemoto() {
    const r = await fetch(API_STATE, { method: 'GET', cache: 'no-store' });
    if (r.status === 503) {
      storageBackend = 'local';
      mostrarToast('Blob não configurado neste ambiente; usando armazenamento local.', 'erro');
  ...
  async function carregar() {
    try {
      ...
      storageBackend = 'local';
      mostrarToast('Não foi possível carregar da nuvem; usando dados locais.', 'erro');
      carregarLocalStorage();
    }
  }
  ...
  window.addEventListener('pagehide', () => {
    ...
    try {
      const body = JSON.stringify(serializarEstado());
      fetch(API_STATE, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json; charset=utf-8' },
        body,
  ...
  (async function iniciar() {
    try {
      ...
      console.error(e);
      mostrarToast('Erro ao iniciar a aplicação.', 'erro');
    }
  })();
})();

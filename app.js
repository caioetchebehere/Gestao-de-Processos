(function () {
  ...
  function aplicarDadosArmazenados(data) {
  ...
  let ultimoPayloadEnviado = '';
  let ultimoJsonRemotoVisto = '';
  ...
  function pushNuvem() {
    if (!nuvemOk) return;
    const json = JSON.stringify(montarPayload());
    if (json === ultimoPayloadEnviado) return;
    fetch('/api/state', {
      method: 'PUT',
      ...
      .then(function (r) {
        if (r.ok) ultimoPayloadEnviado = json;
      })
      .catch(function () {});
  }

  function pushNuvemImediato() {
    if (!nuvemOk) return Promise.resolve();
    const json = JSON.stringify(montarPayload());
  ...
  async function pollNuvem() {
    if (!nuvemOk || document.visibilityState !== 'visible') return;
    try {
      const res = await fetch('/api/state', { cache: 'no-store' });
      if (!res.ok) return;
      const remote = await res.json();
      const sig = remote ? JSON.stringify(remote) : 'null';
      if (sig === ultimoJsonRemotoVisto) return;
      if (!remote || remote.error || !Array.isArray(remote.processos) || remote.processos.length === 0) return;
      ultimoJsonRemotoVisto = sig;
      const localAntes = JSON.stringify(montarPayload());
      aplicarDadosArmazenados(remote);
      const localDepois = JSON.stringify(montarPayload());
      if (localDepois === localAntes) return;
      try {
        salvarLocal();
      } catch (_) {}
      ultimoPayloadEnviado = localDepois;
      renderizarListaProcessos();
      sincronizarUI();
    } catch (_) {}
  }

  async function bootstrapNuvem() {
    try {
      const res = await fetch('/api/state', { cache: 'no-store' });
      if (!res.ok) throw new Error(String(res.status));
      const remote = await res.json();
      if (remote && typeof remote === 'object' && remote.error === 'storage_unavailable') {
        throw new Error('redis');
      }
      nuvemOk = true;
      if (remote && Array.isArray(remote.processos) && remote.processos.length > 0) {
        try {
          aplicarDadosArmazenados(remote);
        } catch (_) {
        ...
        ultimoPayloadEnviado = JSON.stringify(montarPayload());
        ultimoJsonRemotoVisto = JSON.stringify(remote);
        atualizarStatusNuvem('Dados na nuvem: todos com o link veem as mesmas informações.');
      } else {
        carregarDoLocalStorage();
        ultimoPayloadEnviado = JSON.stringify(montarPayload());
        await pushNuvemImediato();
        ultimoJsonRemotoVisto = ultimoPayloadEnviado;
        atualizarStatusNuvem('Dados na nuvem: alterações são compartilhadas entre os computadores.');
      }
    } catch (_) {
      nuvemOk = false;
      carregarDoLocalStorage();
      atualizarStatusNuvem(
        'Somente neste navegador. Para compartilhar entre computadores: no projeto na Vercel, conecte Redis (Upstash) em Storage; em local use vercel dev.'
      );
    }
  }

  // --- Inicialização ---
  bootstrapNuvem().then(function () {
    ...
    atualizarBotaoToggleEtapas();
    if (nuvemOk) {
      setInterval(pollNuvem, 45000);
      document.addEventListener('visibilitychange', function () {
        if (document.visibilityState === 'visible') pollNuvem();
      });
    }
  });
})();
